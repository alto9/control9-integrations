...
- GitLab CI jobs follow the same sequence through the GitLab runner entrypoint. Blocking uses non-zero job exit after structured log output; GitLab job report sections and merge request comments are published by the GitLab presentation path documented separately.
